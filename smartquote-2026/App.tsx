...
const FormatSelectionModal: React.FC<{ onSelect: (type: 'private' | 'government') => void }> = ({ onSelect }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-fade-in print:hidden">
...
const App: React.FC = () => {
  ...
  // Persistence for address book only, others handled in Editor with debounce
  ...
  return (
    <div className="h-screen w-full flex items-center justify-center p-4 md:p-6 overflow-hidden relative print:p-0 print:block print:h-auto print:overflow-visible print:bg-white print:static">
        {showFormatModal && <FormatSelectionModal onSelect={handleSelectFormat} />}

        {/* --- MAIN WORKSPACE --- */}
        <div className="w-full h-full max-w-[1920px] grid grid-cols-1 lg:grid-cols-[480px_1fr] gap-6 relative print:block print:w-full print:h-full print:max-w-none print:contents">
            
            {/* LEFT PANE: EDITOR */}
            <div className="bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-soft flex flex-col overflow-hidden border border-white/40 relative z-20 print:hidden">
                {/* Header */}
                <div className="h-20 px-8 flex items-center justify-between border-b border-slate-100 shrink-0 bg-white">
                    <div className="flex items-center gap-3">
                         ...
                         <div>
                             ...
                             <p className="text-[10px] text-slate-400 font-medium tracking-wide">UBON COMPUTER</p>
                         </div>
                    </div>
            ...
            {/* RIGHT PANE: PREVIEW */}
            <div className="hidden lg:flex flex-col relative rounded-[2rem] bg-slate-900/50 backdrop-blur-sm border border-white/10 shadow-inner overflow-hidden print:flex print:fixed print:inset-0 print:z-[100] print:bg-white print:visible print:overflow-visible print:w-auto print:h-auto print:rounded-none print:border-none print:shadow-none">
                {/* Toolbar */}
                <div className="absolute top-6 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 p-1.5 bg-white/90 backdrop-blur-md rounded-full shadow-xl border border-white/50 no-print print:hidden">
                ...
                {/* Canvas */}
                <div className="flex-1 overflow-y-auto p-12 flex justify-center items-start print:p-0 print:overflow-visible custom-scrollbar print:block print:w-full print:h-auto">
                    <div className="bg-white shadow-2xl print:shadow-none transition-transform duration-300 origin-top scale-[0.9] xl:scale-100 a4-page relative ring-1 ring-black/5 print:transform-none print:ring-0 print:m-0 print:w-[210mm] print:h-[297mm]">
                        <Preview 
