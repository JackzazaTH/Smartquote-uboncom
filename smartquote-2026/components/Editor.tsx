...
const ModernInput: React.FC<{ label: string; icon?: React.ElementType } & React.InputHTMLAttributes<HTMLInputElement>> = ({ label, icon: Icon, className, ...props }) => (
    <div className="group">
        ...
        <div className="relative">
            ...
            <input 
                {...props}
                className={`w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 block p-2.5 transition-all placeholder-gray-300 font-medium ${Icon ? 'pl-9' : ''} ${className}`}
            />
        </div>
    </div>
);
...
const ModernTextArea: React.FC<{ label: string } & React.TextareaHTMLAttributes<HTMLTextAreaElement>> = ({ label, className, ...props }) => (
    <div className="group">
        ...
        <textarea 
            {...props}
            className={`w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 block p-2.5 transition-all placeholder-gray-300 resize-none font-medium ${className}`}
        />
    </div>
);
...
export const Editor: React.FC<EditorProps> = ({
  ...
  return (
    <div className="pb-32 animate-fade-in">
      ...
      {/* 1. Document Settings */}
      <SectionCard title="ตั้งค่าเอกสาร (Document)" icon={Calendar} rightAction={
          <button 
            type="button" 
            onClick={(e) => { e.preventDefault(); setShowDocSettings(!showDocSettings); }} 
            className={`transition-colors p-1.5 rounded-lg ${showDocSettings ? 'bg-brand-100 text-brand-600' : 'text-gray-400 hover:text-brand-500 hover:bg-gray-100'}`}
          >
            <Settings size={18}/>
          </button>
      }>
          ...
          {showDocSettings && (
              <div className="mb-5 p-4 bg-slate-50 border border-slate-200 rounded-lg animate-fade-in text-sm relative">
                  <div className="absolute top-[-6px] right-3 w-3 h-3 bg-slate-50 border-t border-l border-slate-200 transform rotate-45"></div>
                  <h4 className="font-bold text-gray-700 mb-3 flex items-center gap-2 border-b border-gray-200 pb-2">
                      <Settings size={14} /> ตั้งค่าเลขรันอัตโนมัติ (Auto-Run Config)
                  </h4>
                  <div className="grid grid-cols-2 gap-3 mb-3">
                      <ModernInput 
                          label="คำนำหน้า (Prefix)" 
                          value={document.docConfig.prefix} 
                          onChange={e => setDocument({...document, docConfig: {...document.docConfig, prefix: e.target.value}})} 
                      />
                      <div className="group">
                        <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">รูปแบบวันที่</label>
                        <select 
                            value={document.docConfig.dateFormat} 
                            onChange={e => setDocument({...document, docConfig: {...document.docConfig, dateFormat: e.target.value as any}})}
                            className="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 block p-2.5 outline-none"
                        >
                            <option value="YYYYMM">YYYYMM (e.g. 202403)</option>
                            <option value="YYMM">YYMM (e.g. 2403)</option>
                            <option value="YYYY">YYYY (e.g. 2024)</option>
                            <option value="NONE">ไม่ใส่วันที่</option>
                        </select>
                      </div>
                      <ModernInput 
                          type="number" 
                          label="เลขรันปัจจุบัน (Next Run No.)" 
                          value={document.docConfig.runNumber} 
                          onChange={e => setDocument({...document, docConfig: {...document.docConfig, runNumber: parseInt(e.target.value) || 0}})} 
                      />
                      <ModernInput 
                          type="number" 
                          label="จำนวนหลัก (Padding)" 
                          value={document.docConfig.padding} 
                          onChange={e => setDocument({...document, docConfig: {...document.docConfig, padding: parseInt(e.target.value) || 0}})} 
                      />
                  </div>
                  <div className="flex gap-4 border-t border-gray-200 pt-3">
                      <ModernSwitch 
                          checked={document.docConfig.thaiYear} 
                          onChange={c => setDocument({...document, docConfig: {...document.docConfig, thaiYear: c}})} 
                          label="ใช้ปี พ.ศ. (Thai Year)"
                          size="sm"
                      />
                  </div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div className="col-span-2">
                <ModernInput label="พนักงานขาย" icon={User} value={document.preparedBy} onChange={e => setDocument({...document, preparedBy: e.target.value})} />
              </div>
          </div>
