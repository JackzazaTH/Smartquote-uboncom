...
import { Plus, Trash2, Calendar, User, Building, CreditCard, Upload, Image as ImageIcon, X, PenTool, Search, Save, RefreshCw, Box, Tag, Layers, FileText, Lock, Cloud, Loader2, CheckCircle2 } from 'lucide-react';
...
export const Editor: React.FC<EditorProps> = ({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      localStorage.setItem('sq_company', JSON.stringify(company));
      localStorage.setItem('sq_customer', JSON.stringify(customer));
      localStorage.setItem('sq_document', JSON.stringify(document));
      
      const itemsKey = document.formType === 'government' ? 'sq_items_government' : 'sq_items_private';
      localStorage.setItem(itemsKey, JSON.stringify(items));
      
      if (floatingImages) {
        localStorage.setItem('sq_floating_images', JSON.stringify(floatingImages));
      }
  ...
  }, [company, customer, document, items, floatingImages]);
  ...
  return (
    <div className="relative">
      ...
      {/* Sticky Tab Nav */}
      <div className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-100 px-6 py-3 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 pl-4">
               {saveStatus === 'saving' ? (
                   <div className="flex items-center gap-2 text-xs font-bold text-brand-600 animate-pulse">
