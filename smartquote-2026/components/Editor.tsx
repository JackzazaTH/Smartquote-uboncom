    ...
    <div className="space-y-4 pb-20">
      ...
      >
        <div className="space-y-4">
            ...
            >
                <div className={`p-1 rounded-full ${items.length >= 15 ? 'bg-gray-200' : 'bg-brand-100'}`}>
                    <Plus size={18} /> 
                </div>
                {items.length >= 15 ? 'ครบจำนวน 15 รายการแล้ว' : 'เพิ่มรายการใหม่'}
            </button>
            {items.length >= 15 && (
                <div className="flex items-start gap-2 p-3 bg-red-50 text-red-600 text-xs rounded-lg mb-4 border border-red-100">
                    <div className="shrink-0 mt-0.5">⚠️</div>
                    <span>จำกัด 15 รายการ เพื่อให้พอดีกับหน้า A4 หากต้องการเพิ่ม กรุณาลบรายการเดิมออก</span>
                </div>
            )}

            {items.map((item, index) => (
                <div key={item.id} className="relative p-4 bg-white rounded-xl border border-gray-200 shadow-sm group hover:border-brand-300 transition-all hover:shadow-md">
                    <div className="absolute top-0 right-0 p-2 bg-gray-50 rounded-bl-xl border-l border-b border-gray-100 text-[10px] font-bold text-gray-400">
                        #{index + 1}
                    </div>
                    <button 
                        onClick={() => removeItem(item.id)} 
                        className="absolute bottom-2 right-2 text-gray-400 hover:text-red-600 hover:bg-red-50 p-2 rounded-lg transition-colors flex items-center gap-1 text-xs"
                        title="Remove Item"
                    >
                        <Trash2 size={14} /> ลบ
                    </button>
                    ...
                    <div className="grid grid-cols-12 gap-3 pb-2">
                        ...
                        <div className="col-span-12 sm:col-span-8 pr-8">
                             {/* Spacing for index badge */}
                        </div>
